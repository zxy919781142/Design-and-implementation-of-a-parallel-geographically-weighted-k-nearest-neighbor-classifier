	...
	*/
	...
	for(int i = 0;i<N_sample;i++)
	{
		...
		{		
			...
			if(cp==1)
				MPI_Send(&messge,1,MPI_FLOAT,0,tag,MPI_COMM_WORLD);//向主进程发送消息
			if(cp==0)
			{
				MPI_Recv(&messge,1,MPI_FLOAT,1,tag,MPI_COMM_WORLD,&status);//接收从进程发送来的消息
				if(messge<zresorm1)
					printf("选取高斯模型\n");
				else
					printf("选取指数模型\n");
		...
		{		
			...
			if(cp==1)
			{
				zresorm1 = resnorm1(xdata,ydata, point_n);//模型拟合
				zresorm2 = resnorm2(xdata,ydata, point_n);//模型拟合
				MPI_Send(&zresorm1,1,MPI_FLOAT,0,1,MPI_COMM_WORLD);//向主进程发送消息
				MPI_Send(&zresorm2,1,MPI_FLOAT,0,2,MPI_COMM_WORLD);//向主进程发送消息
			}
			//MPI_Barrier(MPI_COMM_WORLD);	
			...
			{
				ts1=MPI_Wtime();
				MPI_Recv(&zresorm1,1,MPI_FLOAT,1,1,MPI_COMM_WORLD,&status);//接收从进程发送来的消息
				MPI_Recv(&zresorm2,1,MPI_FLOAT,1,2,MPI_COMM_WORLD,&status);//接收从进程发送来的消息
				ts2=MPI_Wtime();
				tsz=tsz+ts2-ts1;
